...
<head>
  ...
  <title>Objetivos â€” NutriMax</title>
  <link rel="stylesheet" href="css/styles.css" />
  <style>
    .result-macro-card {
      background:var(--white); border-radius:var(--radius); border:1px solid var(--gray-100);
      padding:20px; text-align:center; transition:var(--transition); animation:fadeInUp .4s ease both;
    }
    .result-macro-card .rm-icon { font-size:32px; margin-bottom:8px; }
    .result-macro-card .rm-val { font-size:32px; font-weight:900; letter-spacing:-1px; }
    .result-macro-card .rm-label { font-size:12px; font-weight:600; color:#111827; text-transform:uppercase; }
    .activity-option {
    ...
    .activity-option:hover { border-color:var(--green-300); background:var(--green-50); }
    .activity-option.selected { border-color:var(--green-500); background:var(--green-50); color:var(--green-700); font-weight:700; }
    .macro-pct-bar { height:10px; border-radius:99px; margin-top:6px; }
  </style>
...
<div class="app-shell">
  ...
  <main class="main-content">
    <div class="page-header mb-32">
      ...
      <p class="page-subtitle">Personaliza tu nutriciÃ³n para optimizar tus resultados</p>
    </div>
    ...
    <div class="grid-2-1" style="gap:24px; align-items:start;">
      ...
      <div style="display:flex; flex-direction:column; gap:24px;">
        <!-- Profile card -->
        <div class="card">
          <div class="card-title">Datos BiomÃ©tricos</div>
          ...
          <div style="display:flex; flex-direction:column; gap:20px; margin-top:20px;">
            ...
            <div class="grid-2" style="gap:16px;">
              <div class="form-group">
                ...
                <input class="form-input" type="number" id="profAge" min="12" max="99" placeholder="25" />
              </div>
              <div class="form-group">
                <label class="form-label">Sexo BiolÃ³gico</label>
      ...
      <div style="display:flex; flex-direction:column; gap:24px;">
        <div class="card">
          ...
          <!-- Custom Inputs -->
          <div id="customGoalInputs" style="display:none; margin-top:16px; flex-direction:column; gap:12px; border-top:1px solid var(--border); padding-top:16px;">
            ...
            <div class="grid-2" style="gap:12px;">
              <div class="form-group">
                ...
                <input class="form-input" type="number" id="cgCals" placeholder="2000" style="padding:8px 12px; font-size:14px;" />
              </div>
              <div class="form-group">
                <label class="form-label" style="font-size:12px;">Proteína (g)</label>
                <input class="form-input" type="number" id="cgProt" placeholder="150" style="padding:8px 12px; font-size:14px;" />
              </div>
            </div>
            <div class="grid-2" style="gap:12px;">
              <div class="form-group">
                <label class="form-label" style="font-size:12px;">Carbos (g)</label>
                <input class="form-input" type="number" id="cgCarbs" placeholder="200" style="padding:8px 12px; font-size:14px;" />
              </div>
              <div class="form-group">
                <label class="form-label" style="font-size:12px;">Grasas (g)</label>
                <input class="form-input" type="number" id="cgFat" placeholder="65" style="padding:8px 12px; font-size:14px;" />
              </div>
            </div>
          </div>
        </div>
        ...
        <div id="resultsSection" style="display:none;">
          <div class="card" style="background:var(--primary); color:white; border:none; margin-bottom:16px;">
            <div style="text-align:center; padding:10px 0;">
              <div style="font-size:12px; font-weight:700; opacity:0.8; text-transform:uppercase; letter-spacing:1px; margin-bottom:8px;">TU TDEE ESTIMADO</div>
              <div style="font-size:48px; font-weight:900; line-height:1;" id="tdeeVal">0</div>
              <div style="font-size:14px; margin-top:4px; opacity:0.9;">kcal / dÃ­a quemadas</div>
            </div>
          </div>

          <div class="card mb-16" style="padding:24px;">
            <div style="font-size:13px; font-weight:600; color:var(--text-muted); margin-bottom:16px; text-transform:uppercase;">PLAN DE MACRONUTRIENTES</div>
            ...
            <button class="btn btn-primary w-full mt-24" onclick="saveGoalsAndNotify()" style="justify-content:center; height:48px;">
    ...
    <div class="option-card ${isSelected ? 'selected' : ''}" onclick="selectGoal('${key}')">
      <div class="option-header">
        <div style="display:flex; align-items:center; gap:12px;">
          <span style="font-size:24px; color:var(--primary);">${cfg.icon}</span>
...
function calculate() {
  ...
  if (selectedGoal !== 'custom') {
    if (!age || !weight || !height) { showToast('Ingresa todos tus datos biométricos', 'error'); return; }
  } else {
    if (!document.getElementById('cgCals').value || !document.getElementById('cgProt').value) {
       showToast('Ingresa tus calorías y proteínas personalizadas', 'error'); return;
    }
  }
  ...
  const macroRows = [
    { label:'CalorÃ­as', val: macros.calories, unit:'kcal', cls:'primary' },
    { label:'ProteÃ­na', val: macros.protein,  unit:'g',    cls:'protein' },
  ...
  const data = [
    { label:'ProteÃ­na', color:'var(--primary)', g: macros.protein, pct: ((macros.protein * 4) / totalCals) * 100 },
...
function saveGoalsAndNotify() {
  ...
  const updated = { ...getUser(),
    name: document.getElementById('profName').value.trim() || user?.name,
  ...
  initSidebar('goals');
  showToast('Â¡ConfiguraciÃ³n guardada con Ã©xito!', 'success');
}
