...
function handleGoogleCredential(response) {
    ...
    .then(data => {
        if (data.status === 'success') {
            ...
            if (typeof loginUserSuccess === 'function') {
                loginUserSuccess(data.user); 
            } else {
                // Si la función no existe en app.js, usamos esta alternativa manual:
                saveUser(data.user); 
                showToast('¡Bienvenido!', 'success');
                setTimeout(() => window.location.href = 'dashboard.php', 800);
            }
        } else if (data.status === 'incomplete') {
            // Lógica para mostrar el modal de datos faltantes
    ...
    .catch(err => console.error("Error en la autenticación:", err));
}
