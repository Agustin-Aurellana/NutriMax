...
<body>
      ...
      <div>
        ...
        <p class="hero-desc">
          ...
          <div class="auth-card" id="mainAuthCard">
            ...
            <button type="button" class="btn-google w-full mb-16" id="googleLoginBtn" onclick="triggerGoogleLogin()">
  ...
  <script>
    ...
    // IMPORTANTE: Reemplaza 'TU_GOOGLE_CLIENT_ID' con tu Client ID real de Google Cloud Console
    ...
    // Dispara el popup de Google cuando el usuario hace clic en el botón
    function triggerGoogleLogin() {
      if (typeof google !== 'undefined' && google.accounts && google.accounts.id) {
        google.accounts.id.prompt();
      } else {
        showToast('Google no disponible. Verifica tu conexión a internet.', 'error');
      }
    }
    ...
    async function handleLogin(e) {
      ...
      try {
        ...
        if (data.status === 'success') {
          const u = data.user;
          let age = 25;
          if (u.nacimiento) age = new Date().getFullYear() - new Date(u.nacimiento).getFullYear();
          const loggedUser = {
      ...
      fetch('google_auth', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
        ...
        .then(data => {
          if (data.status === 'success') {
            ...
            // El usuario es nuevo — lo mandamos a la landing para completar el wizard
            sessionStorage.setItem('nm_google_partial', JSON.stringify(data.partial_user));
            showToast('Completá tu perfil para continuar', 'info');
            setTimeout(() => window.location.href = 'index.php?wizard=1', 600);
          } else {
            showToast(data.message || 'Error al iniciar con Google', 'error');
          }
        })
    ...
    function demoLogin() {
      const demo = {
        name: 'Demo', email: 'demo@nutrimax.com', sex: 'male',
        age: 26, birthDate: '2000-01-01', weight: 75, height: 178,
        activityLevel: 'moderate', goal: 'definition', savedRecipes: []
      };
      saveUser(demo);
